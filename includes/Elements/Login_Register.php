...
class Login_Register extends Widget_Base {
	...
	protected function print_login_form() {
		if ( $this->should_print_login_form ) {
			...
			$default_hide_class = 'register' === $this->default_form || isset($_GET['eael-register']) ? 'ea-d-none' : '';
	...
	protected function print_register_form() {
		if ( $this->should_print_register_form ) {
			$default_hide_class = 'login' === $this->default_form && !isset($_GET['eael-register']) ? 'ea-d-none' : ''; //eael-register flag for show error/success message when formal form submit
